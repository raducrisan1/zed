...
pub fn wrap_with_abbreviation(
    ...
    if build_wrap_targets(editor, &ranges, WrapFilters::default(), cx).is_empty() {
        show_unavailable_toast(editor, cx);
        return;
    }
    ...
    Some(cx.spawn_in(window, async move |editor, cx| {
        ...
        editor
            .update_in(cx, |editor, window, cx| {
                for error in &errors {
                    log::error!("Failed to expand Emmet abbreviation: {error:?}");
                }
                if expansions.is_empty() {
                    let message = match errors.first() {
                        Some(error) => format!("Failed to expand Emmet abbreviation: {error}"),
                        None => format!("No Emmet expansion for {abbreviation:?}"),
                    };
        ...
        editor
            .update(cx, |editor, cx| {
                let preview = match result {
                    ...
                    Err(error) => InlineInputPreview::Error(format!(
                        "Failed to expand Emmet abbreviation: {error}"
                    )),
                };
...
fn show_unavailable_toast(editor: &Editor, cx: &mut App) {
    let message =
        "Wrapping with an Emmet abbreviation is not available for the selected text".to_string();
    log::info!("{message}");
    let Some(workspace) = editor.workspace() else {
        return;
    };
    struct WrapWithAbbreviationToast;
    workspace.update(cx, |workspace, cx| {
        workspace.show_toast(
            Toast::new(
                NotificationId::unique::<WrapWithAbbreviationToast>(),
                message,
            ),
            cx,
        )
...
fn enclosing_element_range(
    ...
    for _ in 0..64 {
        let (node, node_range) = snapshot.syntax_ancestor(range.clone())?;
...
mod tests {
        ...
        editor.update(cx, |editor, cx| {
            ...
            assert_eq!(
                editor
                    ...
                    .as_ref()
                    .and_then(|state| state.preview.lock().clone()),
    ...
    fn select_and_wrap(
        ...
        editor.update_in(cx, |editor, window, cx| {
            editor.change_selections(Default::default(), window, cx, |s| s.select_ranges(ranges));
